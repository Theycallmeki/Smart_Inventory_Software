...
/**
 * Train and persist model for one item
 */
...
/**
 * Predict next quantity using stored model
 */
async function predictItem(itemId) {
  const model = await AiModel.findOne({ where: { itemId } });
  if (!model) return null;

  const { modelType, quantities } = model;

  if (modelType === 'linear-regression') {
    return Math.max(0, linearRegressionPredict(quantities));
  }
...
async function trainModel() {
  try {
    ...
    return '✅ All item models trained and saved to database!';
  } catch (err) {
...
/**
 * Recommend top predicted best sellers
 */
